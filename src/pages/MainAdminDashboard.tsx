...
export default function MainAdminDashboard() {
    ...
    if(jwtDecode<{role:string}>(cookies.auth_token).role === Role.USER.toString()){
          ...
          {[
            ...
            return (
              <motion.div
                ...
                whileTap={{ scale: 1.05 }}
                transition={{ type: "spring", stiffness: 400, damping: 17,delay:.5*index }}
                className={`relative grid grid-cols-2 grid-rows-2 gap-2 p-4 items-center justify-center pb-0 bg-gradient-to-br ${item.color} rounded-2xl shadow-lg shadow-gray-900/30 backdrop-blur-md border border-white/20 transform transition-all duration-300 ease-out hover:shadow-2xl hover:shadow-gray-800/40 cursor-pointer`}
              >
                <h2 className="col-span-1 row-span-1 text-3xl text-start md:text-4xl lg:text-5xl font-bold drop-shadow-xl group-hover:text-gray-200 transition-colors duration-300">
                  {item.value}
                </h2>
                <item.icon className="col-span-1 row-span-1 justify-self-end place-self-end w-16 h-16 mb-2 group-hover:scale-110 transition-transform duration-300 ease-out drop-shadow-lg" />
                <h3 className="col-span-1 row-span-1 p-0 m-0 text-start text-2xl md:text-3xl font-semibold drop-shadow-md group-hover:text-gray-100 transition-colors duration-300">
                  {item.title}
                </h3>
              </motion.div>
            )
            ...
            {usersPerMonthStats?.series && (
              <Suspense 
                fallback={
                  ...
                  className="p-6 rounded-lg shadow-md bg-white dark:bg-gray-800 flex justify-center items-center animate-pulse">
              ...
              >
                <motion.div 
                  ...
                  initial={{ opacity: 0,x:-100 }}
                  whileInView={{ opacity: 1,x:0 }}
                  className="p-6 rounded-lg shadow-md bg-white dark:bg-gray-800 transition-all duration-300 ease-in-out">
            ...
            <tbody>
              {
                communities?.communities.map((community) => (
                  <tr key={community._id} className="hover:bg-gray-300 dark:hover:bg-gray-700">
